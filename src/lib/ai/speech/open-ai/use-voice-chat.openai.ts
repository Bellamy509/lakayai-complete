...
import {
  extractMCPToolId,
  extractMCPToolIdWithServerNames,
} from "lib/ai/mcp/mcp-tool-id";
  ...
  const clientFunctionCall = useCallback(
    async ({
      ...
      if (DEFAULT_VOICE_TOOLS.some((t) => t.name === toolName)) {
        ...
        // Use the enhanced extraction function that handles server names with spaces
        const serverNames = mcpList?.map((server) => server.name) || [];
        const toolId = extractMCPToolIdWithServerNames(toolName, serverNames);
    ...
    [updateUIMessage, mcpList, theme, setTheme],
  );
